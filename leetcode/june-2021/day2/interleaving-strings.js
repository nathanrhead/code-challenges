...
// If we've reached the end of s2, there's no need to iterate through the remainder of s1, but rather only to check whether it matches the remainder of s3.
  ...
  // Setting the base line boolean value.
  let answer = false;
  ...
  if (s3.charAt(k) === s1.charAt(i) && isInterleave(s1, s2, s3, i + 1, j, k + 1)
    || s3.charAt(k) === s2.charAt(j) && isInterleave(s1, s2, s3, i, j + 1, k + 1)) answer = true;

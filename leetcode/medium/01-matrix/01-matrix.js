...
const updateMatrix = function(mat) {
  ...
  let queue = []; // Array of coordinates.
  ...
  while (queue.length > 0) {
    let [r, c] = queue.shift(); // The coordinates, from the queue, of the node whose neighbor's need to be considered.

    // Explore neighbors.
    for (let [dr, dc] of directions) {
      let nr = r + dr, nc = c + dc; // Set the neighbor's coordinates.
